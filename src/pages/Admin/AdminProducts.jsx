...
const AdminProducts = () => {
  ...
  const [variantTab, setVariantTab] = useState("active");
  ...
  const openAdd = () => {
    setForm(emptyForm);
    setModalBrandSearch(""); setModalCatSearch(""); setVariantTab("active");
  ...
  const openView = (product) => {
    setForm({
      ...
      images: product.images?.map((img) => ({
        id: img.id, imageUrl: img.imageUrl, position: img.position,
      })) || [],
  ...
  const handleSave = async () => {
    ...
    try {
      if (modal.mode === "add") {
        ...
        // 2. Set status nếu khác DRAFT (vì createProduct đã set, nhưng đảm bảo)
        if (form.status && form.status !== "DRAFT") {
          await axios.patch(
            `${BASE}/products/${productId}/status?status=${form.status}`,
            {}, { headers: authHeader() },
          );
        }
        ...
        for (const v of form.variants) {
          if (v.option1Value && v.price)
            await axios.post(`${BASE}/products/variants`, {
              productId,
              option1Value: v.option1Value,
              option2Value: v.option2Value || "",
              option3Value: v.option3Value || "",
        ...
        for (const img of form.images) {
          if (img.imageUrl)
            await axios.post(`http://localhost:8080/api/v1/products/images`, {
              productId, imageUrl: img.imageUrl, position: img.position || 1,
            }, { headers: authHeader() });
        }

        // 5. Gắn categories
        for (const catId of form.categoryIds)
          await axios.post(`${BASE}/products/${productId}/categories/${catId}`, {}, { headers: authHeader() });
        ...
        // 2. Cập nhật status nếu thay đổi
        if (form.status !== (modal.product.status || "DRAFT")) {
          await axios.patch(
            `${BASE}/products/${productId}/status?status=${form.status}`,
            {}, { headers: authHeader() },
          );
        ...
        // Xóa variants bị remove khỏi form
        for (const oldId of oldActiveVariantIds) {
          if (!newVariantIds.includes(oldId))
            await axios.delete(`${BASE}/products/variants/${oldId}`, { headers: authHeader() });
        }

        // Upsert variants còn lại
        for (const v of activeFormVariants) {
          ...
          if (v.id)
            await axios.put(`${BASE}/products/variants/${v.id}`, payload, { headers: authHeader() });
          else if (v.option1Value && v.price)
            await axios.post(`${BASE}/products/variants`, payload, { headers: authHeader() });
        }
        ...
        for (const oldId of oldImageIds) {
          if (!newImageIds.includes(oldId))
            await axios.delete(`http://localhost:8080/api/v1/products/images/${oldId}`, { headers: authHeader() });
        }
        for (const img of form.images) {
        ...
        for (const oldId of oldCatIds) {
          if (!form.categoryIds.includes(oldId))
            await axios.delete(`${BASE}/products/${productId}/categories/${oldId}`, { headers: authHeader() });
        }
        for (const catId of form.categoryIds) {
  ...
  const addVariant = () =>
    setForm({
      ...form, variants: [...form.variants, {
        ...
        price: "", stock: "", sku: "", isDeleted: false,
      }]
    });
  ...
  const updateImage = (idx, value) => {
    const u = [...form.images]; u[idx] = { ...u[idx], imageUrl: value };
  ...
  const hasActiveFilter =
    filterName || filterProductId || filterVariantId || filterSku ||
  ...
  const filtered = products.filter((p) => {
    ...
    if (filterProductId && !String(p.id).includes(filterProductId.trim())) return false;
    if (filterBrand && !p.brand?.toLowerCase().includes(filterBrand.toLowerCase())) return false;
    if (filterCategory && !p.categories?.some((c) => c.name.toLowerCase().includes(filterCategory.toLowerCase()))) return false;
    if (filterStatus && p.status !== filterStatus) return false;

    const variants = (p.variants || []).filter((v) => !v.isDeleted);
    if (filterVariantId && !variants.some((v) => String(v.id).includes(filterVariantId.trim()))) return false;
    if (filterSku && !variants.some((v) => (v.sku || "").toLowerCase().includes(filterSku.toLowerCase()))) return false;
  ...
  const deletedVariants = form.variants.filter((v) => v.isDeleted);
  ...
  return (
    <div className="adp-page">
      ...
      <div className="adp-header">
        <div>
          <h2 className="adp-title">Quản lý Sản hẩm</h2>
      ...
      <div className="adp-filter-wrap">
        <div className="adp-filter-bar">
          ...
          <div className="adp-filter-field">
            ...
            <select className="adp-filter-input" value={filterCategory}
          ...
          <div className="adp-filter-field adp-filter-field--range">
            ...
            <div className="adp-filter-range">
              <input className="adp-filter-input" type="number" placeholder="Từ"
                value={filterPriceMin} onChange={(e) => setFilterPriceMin(e.target.value)} />
              <span className="adp-filter-range-sep">—</span>
              <input className="adp-filter-input" type="number" placeholder="Đến"
          ...
          <div className="adp-filter-field adp-filter-field--range">
            ...
            <div className="adp-filter-range">
              <input className="adp-filter-input" type="number" placeholder="Từ"
                value={filterStockMin} onChange={(e) => setFilterStockMin(e.target.value)} />
              <span className="adp-filter-range-sep">—</span>
              <input className="adp-filter-input" type="number" placeholder="Đến"
                value={filterStockMax} onChange={(e) => setFilterStockMax(e.target.value)} />
            </div>
          </div>
        </div>
          ...
          {hasActiveFilter && (
            <>
              <span className="adp-filter-result">
                Tìm thấy <strong>{filtered.length}</strong> / {products.length} sản phẩm
              </span>
        ...
        <table className="adp-table">
          <thead>
            <tr>
              ...
              <th>Giá</th><th>Tồn kho</th><th>Variants</th><th>Thao tác</th>
            </tr>
          </thead>
          <tbody>
            {filtered.length > 0 ? filtered.map((product) => (
              <tr key={product.id} className="adp-row">
                ...
                <td>
                  <div className="adp-cats">
                    {product.categories?.map((c) => (
                      <span key={c.id} className="adp-cat-tag">{c.name}</span>
                    ))}
                  </div>
                </td>
                ...
                <td className="adp-variant-count">
                  {product.variants?.filter((v) => !v.isDeleted).length || 0} phân loại
                </td>
                <td>
                  <div className="adp-actions">
                    {product.status === "DELETED" ? (
                      // Sản phẩm đã xóa: chỉ cho xem, không cho edit/delete
                      <button className="adp-btn-edit" onClick={() => openView(product)}
              ...
              <tr><td colSpan="10" className="adp-empty-row">Không tìm thấy sản phẩm.</td></tr>
            )}
          </tbody>
        </table>
      </div>
          ...
          <div className="adp-modal">
                  ...
                  {filteredModalCats.length === 0 ? (
                    ...
                    filteredModalCats.map((c) => (
                      <span
                        ...
                        className={`adp-cat-chip ${form.categoryIds.includes(c.id)
                          ? "adp-cat-chip--active"
                          : ""
                          }`}
                        style={
            ...
            <div className="adp-section-header">
              <h4 className="adp-section-title">
                Hình ảnh ({form.images.length}) - Giới hạn URL tối đa 500 ký t
              </h4>
            ...
            {form.variants.map((v, idx) => (
              <div key={idx} className="adp-variant-card">
                <div className="adp-form-grid-3">
                  {["option1Value", "option2Value", "option3Value"].map(
                    (field, i) => (
                      <div key={field} className="adp-form-row">
                        <label className="adp-form-label">
                          {[form.option1Name, form.option2Name, form.option3Name][i] ||
                            `Option ${i + 1}`}
                        </label>
                ...
                {!isViewOnly && form.variants.length > 1 && (
                  <button
